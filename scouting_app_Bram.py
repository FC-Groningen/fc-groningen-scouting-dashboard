...
##positions = sorted(df["position"].dropna().unique())
##
##selected_comp = st.sidebar.multiselect("Competition", competitions, default=competitions)
...
### -------- Table with rank, clickable player names, team logos and rounded decimals --------
##table_cols = list(DISPLAY_COLS.keys())
##df_show = df_top[table_cols + ["original_rank"]].copy()
##
### Round all numeric columns to 1 decimal place (except minutes to 0)
##numeric_display_cols = ["age", "total_minutes", "physical", "attack", "defense", "total"]
##for col in numeric_display_cols:
...
##            df_show[col] = df_show[col].round(1)
##
### Create Google search URL column
##df_show["team_search_url"] = df_show["team_name"].apply(get_team_fbref_google_search)
##
### Create team column with embedded base64 logos
##def create_team_html_with_logo(row):
##    team_name = row['team_name']
...
### Reorder columns: rank first, then player name, team with logo, then rest
##cols_order = ["original_rank", "player_name", "team_with_logo_html"] + [c for c in table_cols if c not in ["player_name", "team_name"]]
##df_show = df_show[cols_order + ["team_search_url"]]
##
### Rename columns
##rename_dict = {k: v for k, v in DISPLAY_COLS.items() if k != "team_name"}
##rename_dict["original_rank"] = "#"
...
##        this.eGui = document.createElement('a');
##        this.eGui.innerText = params.value;
##        this.eGui.href = params.data.team_search_url;
##        this.eGui.target = '_blank';
##        this.eGui.style.color = '#1a73e8';
##        this.eGui.style.textDecoration = 'none';
##    }
...
##gb = GridOptionsBuilder.from_dataframe(df_show)
##
### Configure rank column
##gb.configure_column("#", width=60, pinned="left", sortable=True, type=["numericColumn"])
##
### Configure player name with custom renderer
##gb.configure_column("Player Name", width=180, pinned="left", cellRenderer=player_link_renderer)
##
...
##
### Hide the helper column
##gb.configure_column("team_search_url", hide=True)
##
### Set default column properties
##gb.configure_default_column(sortable=True, filterable=False, resizable=True)
##
...
##st.subheader("Player Comparison Charts")
##
##if True:  # Always show since we're using the main dataset
##    # Get available players from the filtered top table (respects all filters)
##    polarized_player_options = sorted(df_top["player_name"].unique().tolist())
##    
##    # Use grid selection as default if available, otherwise empty
##    default_selection = selected_from_grid if selected_from_grid else []
##    
##    selected_players = st.multiselect(
##        "Compare players (max 2) - Select rows in the table above or choose here",
##        options=polarized_player_options,
...
##        st.info("Select players from the table above (using checkboxes) or from the dropdown to view their polarized bar charts.")
##    else:
##        cols = st.columns(2)
##        
##        for i, player_name in enumerate(selected_players):
##            # Get player data from main dataset
...
##
### ========================================
### PLAYER SEARCH & QUICK COMPARISON
### ========================================
##st.markdown("---")
##st.subheader("Player Search & Quick Comparison")
##st.markdown("Search for specific players to compare their performance across different positions and seasons.")
##
### df_search_pool already defined earlier for use by both comparison charts and player search
##available_players = sorted(df_search_pool["player_name"].unique().tolist())
...
##    search_df = df_search_pool[df_search_pool["player_name"].isin(search_selected_players)].copy()
##    
##    # Select and order columns for display (added competition_name)
##    search_cols = ["player_name", "team_name", "position", "competition_name", "season_name", "total_minutes", 
##                   "physical", "attack", "defense", "total"]
##    search_display = search_df[search_cols].copy()
##    
...
##    search_display["team_with_logo_html"] = search_display.apply(create_search_team_html, axis=1)
##    
##    # Create search URL column (keep player name as is for rendering)
##    search_display["player_search_url"] = search_display["team_name"].apply(get_team_fbref_google_search)
##    
##    # Reorder for display (added competition_name)
##    display_cols = ["player_name", "team_with_logo_html", "position", "competition_name", "season_name", 
##                    "total_minutes", "physical", "attack", "defense", "total"]
##    search_display = search_display[display_cols + ["player_search_url"]]
##    
##    # Rename columns
##    search_display = search_display.rename(columns={
##        "player_name": "Player Name",
...
##    gb_search = GridOptionsBuilder.from_dataframe(search_display)
##    
##    # Player name as plain text (no link)
##    gb_search.configure_column("Player Name", width=180, pinned="left")
##    
##    # Team with logo
##    gb_search.configure_column("Team", width=200, cellRenderer=team_logo_renderer)
##    
...
##    
##    # Hide helper column
##    gb_search.configure_column("player_search_url", hide=True)
##    
##    gb_search.configure_default_column(sortable=True, filterable=False, resizable=True)
##    
##    # Enable row selection for search table too
...
##    )
##    
##    # Show message if players selected from search
##    if search_grid_response and 'selected_rows' in search_grid_response:
##        search_selected_rows = search_grid_response['selected_rows']
##        if search_selected_rows is not None and len(search_selected_rows) > 0:
##            # Handle DataFrame or list format
##            if isinstance(search_selected_rows, pd.DataFrame):
##                selected_search_players = search_selected_rows['Player Name'].tolist()[:2]
##            elif isinstance(search_selected_rows, list) and len(search_selected_rows) > 0:
##                if isinstance(search_selected_rows[0], dict):
##                    selected_search_players = [row['Player Name'] for row in search_selected_rows[:2]]
##                else:
##                    selected_search_players = search_selected_rows[:2]
##            else:
##                selected_search_players = []
##            
##            if selected_search_players:
##                # Create comparison charts directly from search selection
##                st.markdown("---")
##                st.subheader("Comparison Charts for Selected Players")
##                
##                if len(selected_search_players) > 2:
##                    st.warning("Please select at most 2 players for comparison.")
##                    selected_search_players = selected_search_players[:2]
##                
##                cols = st.columns(2)
##                
##                for i, player_name in enumerate(selected_search_players):
##                    # Get all matching rows for this player (may have multiple positions/seasons)
##                    player_rows = df_search_pool[df_search_pool["player_name"] == player_name]
##                    
##                    if player_rows.empty:
##                        st.warning(f"No data found for {player_name}")
...
##                        
##                        if team_logo_b64:
##                            caption_html = f'<div style="display: flex; align-items: center; margin-bottom: 1rem;"><img src="{team_logo_b64}" height="24" style="margin-right: 8px;"/><span style="font-size: 1.1rem;">{" | ".join(caption_parts)}</span></div>'
##                        else:
##                            caption_html = f'<div style="font-size: 1.1rem; margin-bottom: 1rem;">{" | ".join(caption_parts)}</div>'
##                        
##                        st.markdown(caption_html, unsafe_allow_html=True)
##                        
##                        # Build radar chart
##                        fig = build_radar_3_shapes(player_data, df_search_pool)
##                        st.plotly_chart(fig, use_container_width=True)
##else:
##    st.info("Select one or more players above to view their detailed statistics across positions and seasons.")
##
##
##
