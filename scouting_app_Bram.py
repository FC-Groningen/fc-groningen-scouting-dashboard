...
def create_polarized_bar_chart(player_data: pd.Series, competition_name: str, season_name: str) -> go.Figure:
    ...
    # Use pre-calculated category averages from the dataframe (if available)
    # This ensures consistency with the 'total' column in the table
    if 'physical' in player_data.index and 'attack' in player_data.index and 'defense' in player_data.index:
        ...
        overall_avg = float(player_data['total']) if 'total' in player_data.index else np.mean([physical_avg, attack_avg, defense_avg])
    else:
        # Fallback: Calculate from individual metrics if category columns not available
